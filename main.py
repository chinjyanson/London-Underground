...
station_names = clist.create_station_list()
coordinates_list = clist.create_coordinates_list()
...
for x in routes:
    ...
    for y in range(len(routes[x])):
        for z in range(len(routes[x][y])-1):
            ...
            time = cost_calc.time_taken(stations_dict[routes[x][y][z]].coordinates, stations_dict[tmpplusone].coordinates, curline)
...
getLoc = geocode.nametocoordinates(start_address)
start_coordinate = coor.get_nearest_coordinate(getLoc[0], getLoc[1])
start = stations_dict[coor.get_nearest_station(start_coordinate)]

stop_address = input("Please enter your destination address: ")
getLoc = geocode.nametocoordinates(stop_address)
stop_coordinate = coor.get_nearest_coordinate(getLoc[0], getLoc[1])
stop = stations_dict[coor.get_nearest_station(stop_coordinate)]

shortest_path = a_star_algo.astar(start, stop, all_stations)
